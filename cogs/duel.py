...
MARTIAL = {'Mega Kick': 100,
           'Sky Uppercut': 85,
           'bitch-slap': 30,
           'Headbutt': 70}


VERB_IND_SUB = {'munch': 'munches', 'toss': 'tosses', 'unleash':'unleashes'}

ATTACK = {"{a} {v} their {o} at {d}!": indicatize(WEAPONS),
          "{a} {v} {o}!": indicatize(MELEE),
          "{a} {v} {o} at {d}!": indicatize(MELEE),
          "{a} {v} {o} at {d}!": indicatize(SINGLE_PROJECTILE),
          "{a} {v} {o} into {d}!": indicatize(SINGLE_PROJECTILE),
          "{a} orders {o} to {v} {d}!": FAMILIAR,
          "{a} summons {o} to {v} {d}!": SUMMON,
          "{a} {v} {d}!": indicatize(MARTIAL),
          "{d} is bowled over by {a}'s sudden Tauros horde!": 60,
          "{a} tickles {d}, causing them to pass out from lack of breath": 25,
          "{a} points at something in the distance, distracting {d} long enough to {v} them!": MARTIAL
          }

CRITICAL = {"Quicker than the eye can follow, {a} delivers a devastating blow with their {o} to {d}.": WEAPONS,
            "{a} {v} {o}! A critical hit!": MELEE,
            "{a} summons {o} to {v} {d}! A critical hit!": SUMMON,
            "{a} used Frost Breath! A critical hit!": 55,
            "{a} {v} {o}! A critical hit!": WEAPONS,
            "{a} orders {o} to {v} {d}. A critical hit!": FAMILIAR,
            "With frightening speed and accuracy, {a} devastates {d} with a Leaf Blade. A critical hit!": 82,
            "{a} {v} {d}. A critical hit!": MARTIAL
            }
...
HEALS = {
    ...
    'drink': {
        ...
        'a': {
            'Super Potion': 60,
            'refreshingly delicious can of 7-Up': 45,
            'Potion': 20
        },
        'an': {
            'Elixir': 80
...
HEAL = {"{a} decides to {v} {o} instead of attacking.": HEALS,
        ...
        "{a} decides to call Chansey to help them with Softboiled.": 150}
...
FUMBLE = {"{a} closes in on {d}, but suddenly remembers a funny joke and laughs instead.": 0,
          ...
          "{a} {v} their {o}, but fumbles and drops it!": indicatize(WEAPONS)
          }
...
MOVES = {'CRITICAL': (CRITICAL, TARGET_OTHER, -1.5),
         'ATTACK': (ATTACK, TARGET_OTHER, -1),
         'FUMBLE': (FUMBLE, TARGET_SELF, -0.75),
         'HEAL': (HEAL, TARGET_SELF, 1),
         'BOT': (BOT, TARGET_OTHER, -9999)}
...
class Duel:
    ...
    async def _duel(self, ctx, user: discord.Member):
        ...
        try:
            ...
            msg.append("Arceus decides %s will go first." % order[0][0])
    ...
    def generate_action(self, attacker, defender, move_cat=None):
        ...
        msg = move.format(a=attacker, d=defender, o=obj, v=verb)
        if hp_delta == 0:
            pass
        else:
            target.hp += hp_delta
