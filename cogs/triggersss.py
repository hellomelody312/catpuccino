...
class TriggerObj:
    def __init__(self, **kwargs):
        ...
        self.server = None # if it's None, the trigger will be implicitly global
        self.channels = kwargs.get("channels", {})
        self.type = kwargs.get("type", "random") # Type of payload. Types: all, random
        self.case_sensitive = kwargs.get("case_sensitive", False)
        self.regex = kwargs.get("regex", True)
    ...
    async def payload(self, author:discord.User):   
        self.responses = [response.replace("<user>", author.mention) for response in self.responses]
        if self.responses:
            self.triggered += 1
        if self.type == "all":
